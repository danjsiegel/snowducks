...
def extract_limit_from_query(query_text: str) -> Tuple[str, Optional[int]]:
    ...
    if match:
        ...
        query_without_limit = re.sub(limit_pattern, '', normalized, flags=re.IGNORECASE).strip()
...
def is_valid_cache_table_name(table_name: str) -> bool:
    ...
    """
    return (table_name.startswith("t_") and 
            len(table_name) == 18 and  # t_ + 16 hex chars
            table_name[2:].isalnum() and  # hex chars only
            table_name[2:].islower())  # lowercase hex only
