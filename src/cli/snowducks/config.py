...
class SnowDucksConfig:
    ...
    ducklake_metadata_path: Path = field(default_factory=lambda: Path.home() / ".snowducks" / "metadata.ducklake")
    ducklake_data_path: Path = field(default_factory=lambda: Path.home() / ".snowducks" / "data")
    ...
    @classmethod
    def from_env(cls, env_file: Optional[str] = None, load_env_file: bool = True) -> "SnowDucksConfig":
        ...
        if missing_params:
            raise ConfigError(f"Missing required environment variables: {', '.join(missing_params)}")
        ...
        if os.getenv("CACHE_FORCE_REFRESH"):
            config.cache_force_refresh = os.getenv("CACHE_FORCE_REFRESH").upper() == "TRUE"
        
        # Cost controls
        if os.getenv("ALLOW_UNLIMITED_EGRESS"):
            config.allow_unlimited_egress = os.getenv("ALLOW_UNLIMITED_EGRESS").upper() == "TRUE"
    ...
    def _detect_auth_method() -> Dict[str, Any]:
        ...
        if os.getenv("SNOWFLAKE_PRIVATE_KEY_PATH"):
            auth_config["snowflake_private_key_path"] = os.getenv("SNOWFLAKE_PRIVATE_KEY_PATH")
            if os.getenv("SNOWFLAKE_PRIVATE_KEY_PASSPHRASE"):
                auth_config["snowflake_private_key_passphrase"] = os.getenv("SNOWFLAKE_PRIVATE_KEY_PASSPHRASE")
            return auth_config
    ...
    def get_snowflake_connection_uri(self) -> str:
        ...
        if self.snowflake_authenticator != "snowflake":
            params.append(f"adbc.snowflake.auth.authenticator={self.snowflake_authenticator}")
        
        if self.snowflake_private_key_path:
            params.append(f"adbc.snowflake.auth.private_key_path={self.snowflake_private_key_path}")
            if self.snowflake_private_key_passphrase:
                params.append(f"adbc.snowflake.auth.private_key_passphrase={self.snowflake_private_key_passphrase}")
    ...
    def get_ducklake_attach_string(self) -> str:
        ...
        if not (self.postgres_host and self.postgres_database and self.postgres_user and self.postgres_password):
            raise ConfigError("PostgreSQL configuration is required for DuckLake metadata.")
    ...
    def validate(self) -> None:
        ...
        if self.snowflake_private_key_path:
            ...
            if not key_path.exists():
                raise ConfigError(f"Private key file not found: {self.snowflake_private_key_path}")
        ...
        if self.postgres_host:
            if not self.postgres_database:
                raise ConfigError("POSTGRES_DATABASE is required when POSTGRES_HOST is set")
            if not self.postgres_user:
                raise ConfigError("POSTGRES_USER is required when POSTGRES_HOST is set")
            if not self.postgres_password:
                raise ConfigError("POSTGRES_PASSWORD is required when POSTGRES_HOST is set")
