...
def process_snowflake_query(con, query):
    ...
    start_match = re.search(r'SELECT\s+\*\s+FROM\s+snowflake_query\s*\(\s*', query, re.IGNORECASE)
    if not start_match:
        return query
    ...
    result = con.execute("SELECT snowflake_query(?, ?, ?)", [query_text, limit, force_refresh])
...
def main():
    ...
    print("🚀 Starting DuckDB interactive session with SnowDucks UDF...", file=sys.stderr)
    ...
    try:
        ...
        if config.postgres_host:
            ...
            con.execute(f"ATTACH '{ducklake_attach_string}' AS snowducks_ducklake (DATA_PATH '{data_path}')")
            
        else:
            # Use local DuckDB file (legacy mode)
            print("📁 Using local DuckDB file for metadata...", file=sys.stderr)
            db_path = subprocess.check_output([sys.executable, "../utils/get_db_path.py"], 
        ...
        if not is_interactive():
            print("⚠️  Non-interactive mode detected. Use interactive mode for best experience.", file=sys.stderr)
        ...
        print("     SELECT * FROM snowflake_query('SELECT * FROM my_table LIMIT 100')", file=sys.stderr)
        print(file=sys.stderr)
        print("  -- With custom limit:", file=sys.stderr)
        print("     SELECT * FROM snowflake_query('SELECT * FROM my_table', 500)", file=sys.stderr)
        print(file=sys.stderr)
        print("  -- Force refresh cache:", file=sys.stderr)
        print("     SELECT * FROM snowflake_query('SELECT * FROM my_table', 1000, true)", file=sys.stderr)
        print(file=sys.stderr)
        print("  -- Use in CTEs and joins:", file=sys.stderr)
        print("     WITH data AS (SELECT * FROM snowflake_query('SELECT ...', 1000, false))", file=sys.stderr)
        print("     SELECT * FROM data WHERE column > 100", file=sys.stderr)
        print(file=sys.stderr)
        print("💡 The UDF works like a real table - use it in FROM, JOIN, CTEs, etc!", file=sys.stderr)
        print(file=sys.stderr)
